...
const getAllDoctorsController = async(req,res) =>{
  try{
    ...
    res.status(500).send({
      success:false,
      error,
      message:"Unable to fetch Doctors Details!",
...
const bookAppointmentController = async(req,res) =>{
  try{
    ...
    res.status(500).send({
      success:false,
      error,
      message:'Cannot Book Appointment at this moment!',
    })
  }
}
...
const bookingAvailabilityController = async (req, res) => {
  try {
    ...
    const fromTime = moment(date).set({
      hour: moment(doctor.timings[0], "HH:mm").hour(),
      minute: moment(doctor.timings[0], "HH:mm").minute(),
    });
    const toTime = moment(date).set({
      hour: moment(doctor.timings[1], "HH:mm").hour(),
      minute: moment(doctor.timings[1], "HH:mm").minute(),
    });

    if (!moment(time).isBetween(fromTime, toTime, null, "[]")) {
      return res.status(200).send({
